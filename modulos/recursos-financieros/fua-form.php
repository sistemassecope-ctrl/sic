<?php
/**
 * Módulo: Formulario de Suficiencia Presupuestal (FUA)
 * Ubicación: /modulos/recursos-financieros/fua-form.php
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = [
        ...
        'tipo_suficiencia' => $_POST['tipo_suficiencia'],
        'estatus' => $_POST['estatus'],
        'resultado_tramite' => $_POST['resultado_tramite'],
        'folio_fua' => trim($_POST['folio_fua']),
        'no_oficio_entrada' => trim($_POST['no_oficio_entrada']),
        'oficio_desf_ya' => trim($_POST['oficio_desf_ya']),
        'clave_presupuestal' => trim($_POST['clave_presupuestal']),
        'monto_obra' => (float) str_replace(',', '', $_POST['monto_obra']),
        'monto_supervision' => (float) str_replace(',', '', $_POST['monto_supervision']),
    ...
    <div class="page-header">
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="fuas.php">Suficiencias</a></li>
                    <li class="breadcrumb-item active">
                        <?= $is_editing ? 'Editar' : 'Capturar' ?> FUA
                    </li>
                </ol>
            </nav>
            <h1 class="page-title"><i class="fas fa-file-invoice-dollar text-primary"></i>
                <?= $is_editing ? 'Editar' : 'Nueva' ?> Suficiencia Presupuestal
            </h1>
        </div>
    </div>
    ...
    <form method="POST" onsubmit="preSubmit()">
        <div class="hoja-papel">
            <div class="sheet-header">
                <div>
                    <h3>SOLICITUD DE SUFICIENCIA (FUA)</h3>
                ...
                <div class="timeline-steps">
                    <div class="step">
                        ...
                        <input type="date" name="fecha_ingreso_admvo" class="form-control form-control-sm"
                            value="<?= $is_editing ? $fua['fecha_ingreso_admvo'] : '' ?>">
                    </div>
                    <div class="step">
                        ...
                        <input type="date" name="fecha_ingreso_cotrl_ptal" class="form-control form-control-sm"
                            value="<?= $is_editing ? $fua['fecha_ingreso_cotrl_ptal'] : '' ?>">
                    </div>
                    <div class="step">
                        <div
                        ...
                        <div class="d-flex gap-1">
                            <input type="date" name="fecha_titular" title="Firma Titular"
                                ...
                                value="<?= $is_editing ? $fua['fecha_titular'] : '' ?>">
                            <input type="date" name="fecha_firma_regreso" title="Regreso de Firma"
                                class="form-control form-control-sm"
                                value="<?= $is_editing ? $fua['fecha_firma_regreso'] : '' ?>">
                        </div>
                    </div>
                    <div class="step">
                        ...
                        <div class="d-flex gap-1">
                            <input type="date" name="fecha_acuse_antes_fa" title="Acuse en SFyA"
                                class="form-control form-control-sm"
                                value="<?= $is_editing ? $fua['fecha_acuse_antes_fa'] : '' ?>">
                            <input type="date" name="fecha_respuesta_sfa" title="Respuesta SFyA"
                                class="form-control form-control-sm"
                                value="<?= $is_editing ? $fua['fecha_respuesta_sfa'] : '' ?>">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-12">
                    ...
                    <textarea name="nombre_proyecto_accion" class="form-control text-uppercase" rows="2"
                        required><?= $is_editing ? e($fua['nombre_proyecto_accion']) : '' ?></textarea>
                </div>

                <div class="col-md-4">
                    <label class="form-label x-small text-muted">TIPO MOVIMIENTO</label>
                    <select name="tipo_suficiencia" class="form-control">
                ...
                <div class="col-md-4">
                    ...
                    <select name="resultado_tramite" class="form-control">
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="importe" id="m_total" class="form-control text-end fw-bold"
                            value="<?= $is_editing ? number_format($fua['importe'], 2) : '' ?>" readonly>
                    </div>
                    <div id="saldo_error" class="text-danger x-small mt-1 fw-bold" style="display:none;">¡Monto excede
                        el saldo del proyecto!</div>
                </div>
                ...
                <div class="col-md-4">
                    <label class="form-label x-small text-muted">FOLIO FUA / OFICIAL</label>
                    <input type="text" name="folio_fua" class="form-control fw-bold text-primary"
                        value="<?= $is_editing ? e($fua['folio_fua']) : '' ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label x-small text-muted">NO. OFICIO ENTRADA</label>
                    <input type="text" name="no_oficio_entrada" class="form-control"
                        value="<?= $is_editing ? e($fua['no_oficio_entrada']) : '' ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label x-small text-muted">NO. OFICIO DESF. YA</label>
                    <input type="text" name="oficio_desf_ya" class="form-control"
                        value="<?= $is_editing ? e($fua['oficio_desf_ya']) : '' ?>">
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" name="clave_presupuestal" class="form-control font-monospace"
                        placeholder="00-00000-0000..." value="<?= $is_editing ? e($fua['clave_presupuestal']) : '' ?>">
                </div>
            ...
            <div class="mt-5 pt-4 border-top text-end">
                <a href="fuas.php" class="btn btn-secondary px-4 me-2">CANCELAR</a>
                <button type="submit" id="btn_submit" class="btn btn-primary px-5 fw-bold">GUARDAR SUFICIENCIA</button>
            </div>
        </div>
    </form>
</main>
...
<script>
    ...
    function calcTotal() {
        ...
        if (total > saldoDisponible) {
            document.getElementById('saldo_error').style.display = 'block';
            document.getElementById('m_total').classList.add('text-danger');
        } else {
            document.getElementById('saldo_error').style.display = 'none';
