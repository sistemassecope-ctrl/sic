...
// Contadores para las tarjetas superiores (Filtrados por área del usuario)
$stats = $pdo->query("
    SELECT 
        SUM(CASE WHEN f.resultado_tramite = 'PENDIENTE' AND f.estatus = 'ACTIVO' THEN 1 ELSE 0 END) as pending,
        SUM(CASE WHEN f.resultado_tramite = 'AUTORIZADO' AND f.estatus = 'ACTIVO' AND f.fecha_ingreso_admvo IS NULL THEN 1 ELSE 0 END) as ready,
        SUM(CASE WHEN (f.fecha_ingreso_admvo IS NOT NULL OR f.fecha_ingreso_cotrl_ptal IS NOT NULL) AND f.fecha_respuesta_sfa IS NULL AND f.estatus = 'ACTIVO' THEN 1 ELSE 0 END) as in_progress,
        SUM(CASE WHEN f.fecha_respuesta_sfa IS NOT NULL AND f.estatus = 'ACTIVO' THEN 1 ELSE 0 END) as committed,
...
// Consulta principal según el TAB
$where = "estatus = 'ACTIVO'";
if ($tab === 'nuevas') {
    $where .= " AND resultado_tramite = 'PENDIENTE'";
} elseif ($tab === 'listas') {
    $where .= " AND resultado_tramite = 'AUTORIZADO' AND fecha_ingreso_admvo IS NULL";
} elseif ($tab === 'proceso') {
    $where .= " AND (fecha_ingreso_admvo IS NOT NULL OR fecha_ingreso_cotrl_ptal IS NOT NULL) AND fecha_respuesta_sfa IS NULL";
} elseif ($tab === 'comprometido') {
    $where .= " AND fecha_respuesta_sfa IS NOT NULL";
}

$sql = "
    SELECT f.*, po.nombre_proyecto, a.nombre_area
    FROM solicitudes_suficiencia f
    LEFT JOIN proyectos_obra po ON f.id_proyecto = po.id_proyecto
    LEFT JOIN areas a ON po.id_unidad_responsable = a.id
    WHERE $where AND ($areaFilter OR f.id_proyecto IS NULL)
...
<main class="main-content">
        ...
        <div class="col-md-3">
            <div class="kpi-card glass-vibrant-bg">
                ...
                <div class="kpi-data">
                    ...
                    <span class="kpi-label">Nuevas por Gestionar</span>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="kpi-card glass-vibrant-bg">
                ...
                <div class="kpi-data">
                    <span class="kpi-value">
                        <?= number_format($stats['ready'] ?? 0) ?>
                    </span>
                    <span class="kpi-label">Listas para Trámite</span>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="kpi-card glass-vibrant-bg">
                ...
                <div class="kpi-data">
                    ...
                    <span class="kpi-label">En Proceso Administrativo</span>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="kpi-card glass-vibrant-bg">
                <div class="kpi-icon warning"><i class="fas fa-money-bill-wave"></i></div>
                <div class="kpi-data">
                    <span class="kpi-value">$
                        <?= number_format(($stats['total_monto'] ?? 0) / 1000000, 1) ?>M
                    </span>
                    <span class="kpi-label">Monto Total en Gestión</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="custom-tabs mb-4">
        <a href="?tab=nuevas" class="tab-item <?= $tab === 'nuevas' ? 'active' : '' ?>">
            <i class="fas fa-envelope-open-text me-2"></i>Por Gestionar
        ...
        <a href="?tab=listas" class="tab-item <?= $tab === 'listas' ? 'active' : '' ?>">
            <i class="fas fa-rocket me-2"></i>Listas para Trámite
        ...
        <a href="?tab=proceso" class="tab-item <?= $tab === 'proceso' ? 'active' : '' ?>">
            <i class="fas fa-tasks me-2"></i>En Seguimiento
        ...
        <a href="?tab=comprometido" class="tab-item <?= $tab === 'comprometido' ? 'active' : '' ?>">
            <i class="fas fa-lock me-2"></i>Comprometido
            <?php if ($stats['committed'] > 0): ?>
            ...
            <?php foreach ($solicitudes as $s):
                ...
                <div class="management-row <?= $priorityClass ?>">
                    ...
                    <div class="row-step step-id">
                        ...
                        <span class="solicitud-folio">#<?= e($s['num_oficio_tramite'] ?: $s['id_fua']) ?></span>
                        <span class="time-elapsed"><i class="far fa-clock me-1"></i>Hace <?= $days ?> días</span>
                    </div>
                    ...
                    <div class="row-step step-actions">
                        <div class="d-flex gap-2">
                            <?php if ($puedeEditar): ?>
                                <a href="solicitud-suficiencia-form.php?id=<?= $s['id_fua'] ?>" class="btn-action-row primary"
                                    title="Gestionar">
