...
$areaFilterSQL = getAreaFilterSQL('e.area_id');

// Total empleados en áreas visibles
$stmt = $pdo->query("SELECT COUNT(*) FROM empleados e WHERE e.estatus = 'Activo' AND e.activo = 1 AND $areaFilterSQL");
...
$empleadosPorArea = $pdo->query("
    ...
    FROM areas a
    LEFT JOIN empleados e ON a.id = e.area_id AND e.estatus = 'Activo' AND e.activo = 1
...
$empleadosPorPuesto = $pdo->query("
    SELECT p.nombre as nombre_puesto, COUNT(e.id) as total
    FROM puestos_trabajo p
    LEFT JOIN empleados e ON p.id = e.puesto_trabajo_id AND e.estatus = 'Activo' AND e.activo = 1 AND $areaFilterSQL
    WHERE p.activo = 1
...
<main class="main-content">
    ...
    <div class="card"
        style="margin-bottom: 1.5rem; background: linear-gradient(135deg, rgba(240, 136, 62, 0.1) 0%, rgba(248, 81, 73, 0.1) 100%);">
                    ...
                    <tbody>
                        ...
                        foreach ($empleadosPorArea as $item):
                            ...
                            <tr>
                                ...
                                <td>
                                    <div
                                        style="background: var(--bg-tertiary); border-radius: 4px; height: 20px; overflow: hidden;">
                                        <div
                                            style="background: var(--gradient-primary); width: <?= $porcentaje ?>%; height: 100%; transition: width 0.5s ease;">
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div
                                        style="background: var(--bg-tertiary); border-radius: 4px; height: 20px; overflow: hidden;">
                                        <div
                                            style="background: var(--gradient-success); width: <?= $porcentaje ?>%; height: 100%; transition: width 0.5s ease;">
                                        </div>
                                    </div>
                                </td>
                            </tr>
    ...
    <div class="card" style="margin-top: 1.5rem;">
        <div class="card-body" style="display: flex; align-items: center; gap: 1rem;">
            <div
                style="width: 48px; height: 48px; background: rgba(88, 166, 255, 0.15); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
