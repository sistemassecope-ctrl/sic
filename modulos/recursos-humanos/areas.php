...
$tieneAccesoRH = isAdmin();
if (!$tieneAccesoRH) {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM usuario_modulo_permisos ump INNER JOIN modulos m ON ump.id_modulo = m.id WHERE ump.id_usuario = ? AND (m.id_padre = 2 OR m.id = 2)");
    $stmt->execute([$_SESSION['user_id']]);
    $tieneAccesoRH = $stmt->fetchColumn() > 0;
}
...
$permisos = getUserPermissions(32); 
if (empty($permisos)) $permisos = getUserPermissions(20);

$puedeVer = isAdmin() || in_array('ver', $permisos);
$puedeCrear = isAdmin() || in_array('crear', $permisos);
$puedeEditar = isAdmin() || in_array('editar', $permisos);
$puedeEliminar = isAdmin() || in_array('eliminar', $permisos);
...
if (!$mostrarInactivas) $sql .= " AND a.estado = 1";
...
if ($busqueda) {
    ...
    function organizarAreasJerarquicas($areas, $padreId = null, &$resultado = []) {
        foreach ($areas as $area) {
            ...
            if ($padreId === null) $esHijo = empty($area['area_padre_id']);
            if ($esHijo) {
                $resultado[] = $area;
    ...
    if (count($areas) < count($allAreas)) {
        ...
        foreach ($allAreas as $a) {
            if (!in_array($a['id'], $idsProcesados)) $areas[] = $a;
        }
    }
}
...
<main class="main-content">
    <div class="page-header">
        <div>
            <h1 class="page-title"><i class="fas fa-sitemap" style="color: var(--accent-primary);"></i> Gestión de Áreas</h1>
        ...
        <div style="display: flex; gap: 0.5rem;">
            <a href="?<?= $mostrarInactivas ? '' : 'inactivas=1' ?>" class="btn btn-secondary">
                <i class="fas fa-<?= $mostrarInactivas ? 'eye' : 'eye-slash' ?>"></i> <?= $mostrarInactivas ? 'Ocultar inactivas' : 'Mostrar inactivas' ?>
            </a>
            <?php if ($puedeCrear): ?>
            <button type="button" class="btn btn-primary" onclick="document.getElementById('modalCrear').style.display='flex'">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>Estado</th>
                            <?php if ($puedeEditar || $puedeEliminar): ?> <th style="width: 150px;">Acciones</th> <?php endif; ?>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (empty($areas)): ?>
                            <tr><td colspan="6" style="text-align: center; padding: 3rem;">No se encontraron áreas</td></tr>
                        <?php else: ?>
                            ...
                            <?php 
                                ...
                                if ($area['estado'] == 0) $rowStyle = 'color: #6b7280; opacity: 0.6;';
                            ?>
                            <tr style="<?= $rowStyle ?>">
                                <td style="<?= $nameStyle ?>">
                                    <?php 
                                    ...
                                    <div style="display: flex; align-items: center;">
                                        <?php if ($indent > 0): ?>
                                            <span style="display: inline-block; width: <?= $indent * 25 ?>px; border-left: 1px dashed currentColor; height: 25px; margin-right: 8px; opacity: 0.3;"></span>
                                ...
                                <td style="text-align: center;">
                                    <?php if ($area['total_empleados'] > 0): ?>
                                        <a href="#" 
                                           ...
                                           class="badge badge-info hover-effect" 
                                           title="Ver lista de empleados">
                                            <i class="fas fa-users" style="font-size: 0.7em; margin-right: 3px;"></i> <?= $area['total_empleados'] ?>
                                        </a>
                                    <?php else: ?>
                                        <span class="badge badge-secondary" style="opacity: 0.5;">0</span>
                                    <?php endif; ?>
                                </td>
                                <td><span class="badge <?= $area['estado'] == 1 ? 'badge-success' : 'badge-danger' ?>"><?= $area['estado'] == 1 ? 'Activa' : 'Inactiva' ?></span></td>
                                <?php if ($puedeEditar || $puedeEliminar): ?>
                                <td>
                                    <!-- Reset color for buttons -->
                                    <div style="display: flex; gap: 0.5rem; color: var(--text-primary);">
                                        <?php if ($puedeEditar): ?>
                                        <a href="?editar=<?= $area['id'] ?>" class="btn btn-sm btn-secondary"><i class="fas fa-edit"></i></a>
                                        <?php endif; ?>
                                        <?php if ($area['estado'] == 1 && $puedeEliminar): ?>
                                        <form method="POST" style="display: inline;" onsubmit="return confirm('¿Desactivar?')">
                                            <input type="hidden" name="accion" value="eliminar"><input type="hidden" name="id" value="<?= $area['id'] ?>">
                                            <button type="submit" class="btn btn-sm btn-danger"><i class="fas fa-ban"></i></button>
                                        </form>
                                        <?php elseif ($area['estado'] == 0 && $puedeEditar): ?>
                                        <form method="POST" style="display: inline;">
                                            <input type="hidden" name="accion" value="activar"><input type="hidden" name="id" value="<?= $area['id'] ?>">
...
<div id="modalCrear" class="modal-overlay" style="display: none;">
    <div class="modal-content">
        <div class="modal-header"><h3>Nueva Área</h3><button class="modal-close" onclick="this.closest('.modal-overlay').style.display='none'">&times;</button></div>
        <form method="POST">
            <input type="hidden" name="accion" value="crear">
            <div class="modal-body">
                <div class="form-group"><label class="form-label">Nombre *</label><input type="text" name="nombre_area" class="form-control" required></div>
                <div class="form-group"><label class="form-label">Tipo *</label>
                    <select name="id_tipo_area" class="form-control" required>
                        <option value="">Seleccione...</option>
                        <?php foreach ($tiposArea as $t): ?><option value="<?= $t['id'] ?>"><?= e($t['nombre_tipo']) ?></option><?php endforeach; ?>
                    </select>
                </div>
                <div class="form-row" style="display: grid; grid-template-columns: 2fr 1fr; gap: 1rem;">
                    <div class="form-group"><label class="form-label">Área Superior</label>
                        <select name="area_padre_id" class="form-control">
                            ...
                            <?php foreach ($areasParaPadre as $ap): ?><option value="<?= $ap['id'] ?>"><?= e($ap['nombre_area']) ?></option><?php endforeach; ?>
                        </select>
                    </div>
                </div>
                <div class="form-group"><label class="form-label">Nivel</label><input type="number" name="nivel" class="form-control" value="1" min="1"></div>
            </div>
            <div class="modal-footer"><button type="button" class="btn btn-secondary" onclick="this.closest('.modal-overlay').style.display='none'">Cancelar</button><button type="submit" class="btn btn-primary">Guardar</button></div>
        </form>
    </div>
</div>
<?php endif; ?>
...
<div id="modalEditar" class="modal-overlay" style="display: flex;">
    <div class="modal-content">
        <div class="modal-header"><h3>Editar Área</h3><a href="?" class="modal-close">&times;</a></div>
        <form method="POST">
            <input type="hidden" name="accion" value="editar"><input type="hidden" name="id" value="<?= $areaEditar['id'] ?>">
            <div class="modal-body">
                <div class="form-group"><label class="form-label">Nombre *</label><input type="text" name="nombre_area" class="form-control" required value="<?= e($areaEditar['nombre_area']) ?>"></div>
                <div class="form-group"><label class="form-label">Tipo *</label>
                    <select name="id_tipo_area" class="form-control" required>
                        <option value="">Seleccione...</option>
                        <?php foreach ($tiposArea as $t): ?><option value="<?= $t['id'] ?>" <?= $areaEditar['id_tipo_area'] == $t['id'] ? 'selected' : '' ?>><?= e($t['nombre_tipo']) ?></option><?php endforeach; ?>
                    </select>
                </div>
                <div class="form-row" style="display: grid; grid-template-columns: 2fr 1fr; gap: 1rem;">
                    <div class="form-group"><label class="form-label">Área Superior</label>
                ...
                <div class="form-group"><label class="form-label">Nivel</label><input type="number" name="nivel" class="form-control" value="<?= e($areaEditar['nivel']) ?>" min="1"></div>
            </div>
            <div class="modal-footer"><a href="?" class="btn btn-secondary">Cancelar</a><button type="submit" class="btn btn-primary">Guardar</button></div>
        </form>
    </div>
</div>
<?php endif; ?>
...
<style>
.modal-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); display: flex; alignItems: center; justifyContent: center; z-index: 2000; }
.modal-content { background: var(--bg-card); border: 1px solid var(--border-primary); border-radius: var(--radius-lg); width: 100%; max-width: 500px; max-height: 90vh; overflow-y: auto; }
.modal-header { padding: 1.25rem 1.5rem; border-bottom: 1px solid var(--border-primary); display: flex; align-items: center; justify-content: space-between; }
.modal-header h3 { margin: 0; font-size: 1.1rem; }
.modal-close { background: none; border: none; fontSize: 1.5rem; color: var(--text-muted); cursor: pointer; textDecoration: none; }
...
<div id="modalEmpleados" class="modal-overlay" style="display: none; backdrop-filter: blur(5px);">
    <div class="modal-content" style="max-width: 700px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
        <div class="modal-header" style="background: linear-gradient(to right, #1e293b, #0f172a);">
            ...
            <button class="modal-close" onclick="document.getElementById('modalEmpleados').style.display='none'" style="color: #cbd5e1;">&times;</button>
        </div>
        <div class="modal-body" style="padding: 0; max-height: 60vh; overflow-y: auto; background: #0f172a;">
            ...
            <table class="table table-hover" id="tablaEmpleadosModal" style="margin: 0; display: none; width: 100%;">
                <thead style="background: #1e293b; position: sticky; top: 0;">
                    <tr>
                        <th style="padding: 12px 20px; color: #94a3b8; font-weight: 500; font-size: 0.85rem;">NO. EMP</th>
                        <th style="padding: 12px 20px; color: #94a3b8; font-weight: 500; font-size: 0.85rem;">NOMBRE</th>
                        <th style="padding: 12px 20px; color: #94a3b8; font-weight: 500; font-size: 0.85rem;">PUESTO</th>
        ...
        <div class="modal-footer" style="background: #1e293b; border-top: 1px solid rgba(255,255,255,0.05);">
            <button type="button" class="btn btn-secondary" onclick="document.getElementById('modalEmpleados').style.display='none'">Cerrar</button>
        </div>
    </div>
</div>
...
.hover-effect { cursor: pointer; transition: all 0.2s; border: 1px solid transparent; }
.hover-effect:hover { transform: scale(1.1); box-shadow: 0 0 10px rgba(59, 130, 246, 0.5); background-color: var(--accent-primary); border-color: rgba(255,255,255,0.3); }

/* Estilos para la tabla del modal */
#tablaEmpleadosModal tbody tr { cursor: pointer; transition: background 0.2s; border-bottom: 1px solid rgba(255,255,255,0.03); }
#tablaEmpleadosModal tbody tr:hover { background: rgba(59, 130, 246, 0.1); }
#tablaEmpleadosModal tbody td { padding: 12px 20px; vertical-align: middle; color: #e2e8f0; font-size: 0.9rem; }
.emp-name { font-weight: 500; color: #fff; }
.emp-id { font-family: 'Courier New', monospace; color: var(--accent-primary); }
.emp-position { font-size: 0.85rem; color: #94a3b8; }
</style>
