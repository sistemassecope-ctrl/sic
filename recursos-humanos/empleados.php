...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar']) && $puedeEliminar) {
    ...
    $stmt = $pdo->prepare("SELECT id_area FROM empleados WHERE id = ?");
    $stmt->execute([$empleadoId]);
    $empleado = $stmt->fetch();
    
    if ($empleado && in_array($empleado['id_area'], $areasUsuario)) {
        $stmt = $pdo->prepare("UPDATE empleados SET estado = 0 WHERE id = ?");
        $stmt->execute([$empleadoId]);
...
<main class="main-content">
    ...
    <div class="card" style="margin-bottom: 1.5rem; background: linear-gradient(135deg, rgba(88, 166, 255, 0.1) 0%, rgba(163, 113, 247, 0.1) 100%);">
                            ...
                            <tr data-id="<?= $emp['id'] ?>">
                                <td>
                                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                                        <span style="width: 40px; height: 40px; background: var(--gradient-accent); 
                                ...
                                <td>
                                    <div style="font-size: 0.85rem;">
                                        <?php if ($emp['email']): ?>
                                            <div><i class="fas fa-envelope" style="width: 16px; color: var(--text-muted);"></i> <?= e($emp['email']) ?></div>
                                        <?php endif; ?>
                                        <?php if ($emp['telefono']): ?>
                                            <div><i class="fas fa-phone" style="width: 16px; color: var(--text-muted);"></i> <?= e($emp['telefono']) ?></div>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td>
                                    <span class="badge <?= $emp['estado'] == 1 ? 'badge-success' : 'badge-danger' ?>">
                                ...
                                <td>
                                    <div style="display: flex; gap: 0.5rem;">
                                        ...
                                        <form method="POST" style="display: inline;" 
                                            ...
                                            <button type="submit" name="eliminar" class="btn btn-sm btn-danger" title="Desactivar">
