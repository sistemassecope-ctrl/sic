...
function tieneAccesoRegistro($modulo, $registroAreaId, $registroCreadorId = null)
{
    ...
    if (!$permiso)
        return false; // No tiene permiso ni de ver
...
function obtenerPadreArea($areaId)
{
    ...
    $stmt = $pdo->prepare("SELECT area_padre_id FROM area WHERE id = ?");
    $stmt->execute([$areaId]);
...
{
    if (!isset($_SESSION['user_id']))
        return " AND 0 "; // Sin sesión = 0 resultados
...
function obtenerIdsJerarquia($rootAreaId)
{
    ...
    $sql = "SELECT id FROM area WHERE area_padre_id IN (" . implode(',', $ids) . ")";
    $stmt = $pdo->query($sql);
    ...
    while (!empty($hijos)) {
        ...
        if (empty($nuevosHijos))
            break;

        // Buscar hijos de los nuevos hijos
        $sql = "SELECT id FROM area WHERE area_padre_id IN (" . implode(',', $nuevosHijos) . ")";
        $stmt = $pdo->query($sql);
        $hijos = $stmt->fetchAll(PDO::FETCH_COLUMN);
    }
