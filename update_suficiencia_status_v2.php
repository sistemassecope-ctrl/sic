...
try {
    ...
    $stmtCheck = $pdo->query("SHOW COLUMNS FROM solicitudes_suficiencia LIKE 'id_momento_gestion'");
    if (!$stmtCheck->fetch()) {
        $pdo->exec("ALTER TABLE solicitudes_suficiencia ADD COLUMN id_momento_gestion INT DEFAULT 1 AFTER estatus");
        // Intentar agregar FK (opcional para evitar errores de motor)
        try {
            ...
            echo "Aviso: No se pudo crear FK (evitable): " . $eFK->getMessage() . "\n";
        }
    }
